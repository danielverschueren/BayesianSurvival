...
if __name__ == "__main__":
    ...
    try:
        ...
        pass
    file = "data/PBMonoOS_TRRW_start.csv"
    ...
    if load:
        with open(
            Path(save_dir, f"PosteriorWeibullpembro_{date}.pk"), 'rb'
        ) as f:
        ...
        # save runs
        with open(
            Path(save_dir, f"PosteriorWeibullpembro_{date}.pk"), 'wb'
        ) as handle:
    ...
    mcmc_stats.to_csv(Path(save_dir,f"pembro_mcmc_stats_{date}.csv"))

    # mcmc check
    az.plot_trace(PosteriorsT[50][1])
    plt.savefig(
        Path(save_dir, f"pembro_traceplot_t{PosteriorsT[50][0]}_{date}.png"), 
        bbox_inches="tight"
    )
    plt.savefig(
        Path(save_dir, f"pembro_traceplot_t{PosteriorsT[50][0]}_{date}.svg"), 
        bbox_inches="tight"
    )
    plt.close()
    ...
    with model:
        ...
        plt.savefig(
            Path(save_dir, f"pembro_priors_{date}.png"), 
            bbox_inches="tight"
        )
        plt.savefig(
            Path(save_dir, f"pembro_priors_{date}.svg"), 
            bbox_inches="tight"
        )
        plt.close()
    ...
    plt.savefig(
        Path(save_dir, f"pembro_kms_priorcheck_t{PosteriorsT[-1][0]}_{date}.png"), 
        bbox_inches="tight"
    )
    plt.savefig(
        Path(save_dir, f"pembro_kms_priorcheck_t{PosteriorsT[-1][0]}_{date}.svg"), 
        bbox_inches="tight"
    )
    plt.close()
    ...
    plt.savefig(
        Path(save_dir, f"pembro_posteriors_{date}.png"), 
        bbox_inches="tight"
    )
    plt.savefig(
        Path(save_dir, f"pembro_posteriors_{date}.svg"), 
        bbox_inches="tight"
    )
    plt.close()
    ...
    for i,j in enumerate(show_js):
        ...
        plt.xticks(xticks, xticks)
        plt.savefig(
            Path(save_dir, f"pembro_kms_posteriorcheck_t{PosteriorsT[j][0]}_{date}.png"), 
            bbox_inches="tight"
        )
        plt.savefig(
            Path(save_dir, f"pembro_kms_posteriorcheck_t{PosteriorsT[j][0]}_{date}.svg"), 
            bbox_inches="tight"
        )
        plt.close()
    ...
    plt.savefig(
        Path(save_dir, f"pembro_bfs_inbetween_{date}.png"), 
        bbox_inches="tight"
    )
    plt.savefig(
        Path(save_dir, f"pembro_bfs_inbetween_{date}.svg"), 
        bbox_inches="tight"
    )
    plt.close()
